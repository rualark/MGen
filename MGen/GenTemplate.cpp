...
void CGenTemplate::SaveResults(CString dir, CString fname)
{
	...
	fs << st;
	st.Format("t_sent = %d\n # Number of steps I finished and sent (can be less than generated)", t_sent);
	fs << st;
	st.Format("t_send = %d # Send window\n", t_send);
	fs << st;
	st.Format("ng_min = %d\n # Minimum generated note", ng_min);
	fs << st;
	st.Format("ng_max = %d\n # Maximum generated note", ng_max);
	fs << st;
	st.Format("tg_min = %d\n # Minimum generated tempo", tg_min);
	fs << st;
	st.Format("tg_max = %d\n # Maximum generated tempo", tg_max);
	fs << st;
	st.Format("time_started = %d # Unix timestamp of generation start\n", time_started);
	fs << st;
	st.Format("time_stopped = %d\n # Unix timestamp of generation finish", time_stopped);
	fs << st;
