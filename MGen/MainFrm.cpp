...
void CMainFrame::LoadResults(CString path) {
	...
	if (pGen != 0) {
		...
		if (pGen->save_format_version != MGR_VERSION) MessageBox("This file was created with a different file format " + pGen->save_format_version + "\nYou can get errors trying to load it. This application version " + APP_VERSION + " works with file format version " + MGR_VERSION + ". Loading of older file formats is usually supported.", "Error");
