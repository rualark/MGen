Instruments = Violin,Piano,Piano,Piano,Piano,Piano,Piano,Piano,Piano,Piano

#Midi_file = "midi\not-midi.mid" # Path and file name of imported midi file
#Midi_file = "midi\GenMP1\test-legato.mid" # Path and file name of imported midi file
#Midi_file = "midi\GenMP1\test-stac.mid" # Path and file name of imported midi file
Midi_file = "midi\GenMP1\test-retrigger.mid" # Path and file name of imported midi file

adapt_enable = 1 # 1 if you want to adapt voices to instruments
midifile_in_mul = 8 # How much you want to decrease minimum step length from 1/8

Instruments = Horn
